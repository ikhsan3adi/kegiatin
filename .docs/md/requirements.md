...
| FR-04 | UR-02        | Admin dapat membuat kegiatan baru dengan parameter: Judul, Deskripsi, Tipe (Single/Series), Lokasi, Narahubung, Gambar, Visibilitas (Open/Invite Only), dan daftar sesi.      | Judul, Deskripsi, Tipe, Lokasi, Narahubung, Gambar (URL dari upload), Visibilitas, Sessions[]. | Data kegiatan tersimpan berstatus DRAFT, Notifikasi Berhasil Simpan. |
| FR-05 | UR-02        | Untuk Series Event, client menghitung tanggal sesi secara otomatis berdasarkan pola pengulangan (Mingguan, Bulanan, atau Custom/input jumlah pertemuan). User dapat meninjau dan mengedit tanggal tiap sesi sebelum submit. | Pola (Mingguan/Bulanan/Custom), Jumlah sesi, Tanggal mulai. | Preview daftar sesi tergenerate, bisa diedit per sesi sebelum submit. |
...
| NFR-05 | FR-07        | Security       | QR Token RSVP harus unik per peserta per event dan tidak dapat dipalsukan atau digunakan ulang di sesi berbeda.                       | QR Token menggunakan custom algorithm (dirancang tim), satu token per RSVP, single-use per sesi. Server dapat memvalidasi token secara deterministik. |
